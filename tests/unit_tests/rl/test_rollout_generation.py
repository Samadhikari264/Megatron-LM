...
class TestGroupedRollouts:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # two complete rounds' worth, env "b" six, so balanced rounds bank
            # and the can_skip_inference read finds one ready.
            pytest.param(0, 6, 4, True, id="stalled_engine_banks_complete_rounds"),
        ],
    )
    async def test_group_consumption_balances_each_batch(
